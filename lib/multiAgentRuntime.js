...
const DEFAULT_WORKFLOW_RELATIVE = path.join(__dirname, '..', 'workflow-templates', 'minimal_txt2img_api.json');
...
async function runComfyWorker(task, comfyBase, workflowPath, positiveNodeId, samplerNodeId, comfyTimeoutMs) {
    if (!comfyBase) {
    ...
    const sNode = String(samplerNodeId);
    if (w[sNode] && w[sNode].inputs && typeof w[sNode].inputs.seed === 'number') {
        w[sNode].inputs.seed = Math.floor(Math.random() * (2 ** 31 - 2)) + 1;
    }
...
async function executeMultiAgentRun(options) {
    const {
        ...
        positiveNodeId = process.env.COMFYUI_POSITIVE_NODE_ID || '6',
        samplerNodeId = process.env.COMFYUI_SAMPLER_NODE_ID || '3',
        maxTasks = Math.min(
